...
| `DEFAULT_ENGINE_HOST` | `"127.0.0.1"`              | No       | Loopback boundary for the managed desktop sidecar. Exposing it would let local config point generated sidecar internals at a remote host. | — | Host fallback used when rendering the managed sidecar config from an incomplete engine URL. |
| `DEFAULT_ENGINE_HTTP_PORT` | `8080` | No | Mirrors the bundled sidecar diagnostics/readiness listener. Users tune the full `http_url` instead of this internal fallback. | — | HTTP diagnostics port used when an engine HTTP URL omits a port. |
| `DEFAULT_ENGINE_GRPC_PORT` | `50051` | No | Mirrors the bundled sidecar gRPC listener. Users tune the full `grpc_url` instead of this internal fallback. | — | gRPC port used when an engine gRPC URL omits a port. |
| `DEFAULT_ENGINE_DAEMON_PORT` | `50061` | No | Internal Go-control-plane to Rust-daemon port; exposing it would require coordinating two processes and firewall expectations. | — | Port written into the managed sidecar config for the Rust daemon service. |
| `DEFAULT_CONTEXT_SERVICE_PORT` | `9191` | No | Internal context-service listener consumed by the sidecar config; changing it without changing rag-engine breaks local context writes. | — | Port used in the generated context service URL. |
| `DEFAULT_ENGINE_HISTORY_WINDOW_TURNS` | `12` | No | Prompt budget contract for engine chat. Larger windows risk crowding out local RAG context; smaller windows degrade continuity. | — | Number of recent chat messages included when building the engine prompt. |
| `DEFAULT_ENGINE_LOCAL_CONTEXT_MAX_SNIPPET_CHARS` | `900` | No | Prompt safety cap on retrieved local snippets. Exposing it would let a malformed index flood the model context. | — | Maximum characters copied from each local RAG source into the augmented prompt. |
| `DEFAULT_ENGINE_RUNTIME_TEMPERATURE` | `1.0` | No | Matches the existing Ollama sampling behavior so switching backends does not silently change answer style. | — | Temperature passed to rag-engine runtime inference. |
| `DEFAULT_ENGINE_RUNTIME_TOP_P` | `0.95` | No | Matches the existing Ollama sampling behavior and should move only with a broader model-quality retune. | — | Nucleus sampling value passed to rag-engine runtime inference. |
| `DEFAULT_ENGINE_RUNTIME_TOP_K` | `64` | No | Matches the existing Ollama sampling behavior and should move only with a broader model-quality retune. | — | Top-k sampling value passed to rag-engine runtime inference. |
